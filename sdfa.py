...
class SDFA():
    def __init__(self, model_path, gpu=-1):
        ...
        if gpu < 0:
            ...
            model_dict = torch.load(self.model_path, map_location=lambda storage, loc: storage.cuda(gpu))
        ...
        self.encoder_id = ImageEncoder(code_size=self.id_enc_dim, img_size=self.img_size) 
    ...
    def preprocess_img(self, img):
        ...
        warped = tf.warp(img, inverse_map=tform.inverse, output_shape=self.img_size)  
        warped = warped * 255  
